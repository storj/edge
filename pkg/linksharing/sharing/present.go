...
func (handler *Handler) showObject(ctx context.Context, w http.ResponseWriter, r *http.Request, pr *parsedRequest, project *uplink.Project, o *uplink.Object, d io.ReadCloser, httpRange httpranger.HTTPRange) (err error) {
	...
	if (download || !wrap) && !mapOnly {
		if len(archivePath) > 0 { // handle zip archives
			...
			acceptsGz := httpcoding.IsAcceptable(httpcoding.Gzip, r.Header)
			if !acceptsGz && !httpcoding.IsAcceptable(httpcoding.Identity, r.Header) {
				w.Header().Set("Accept-Encoding", fmt.Sprintf("%s, %s, *;q=0", httpcoding.Gzip, httpcoding.Identity))
			...
			if isGz {
				w.Header().Set("Content-Encoding", httpcoding.Gzip)
			}
